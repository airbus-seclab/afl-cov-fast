...
def perform_env_check(args: argparse.Namespace):
    ...
    path = args.afl_path / "afl-frida-trace.so"
    if not path.is_file():
        raise ValueError(f"{path} file not found, did you compile AFL-Frida?")
...
async def generate_coverage(
    ...
    async with semaphore:
        ...
        # We use environment variables to enable Frida stalker and write
        # coverage to the generated file path
        # Note: We keep the original environment variables as afl-frida-trace
        # accepts many AFL_* options through there
